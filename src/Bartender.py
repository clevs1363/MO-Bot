...
class Bartender(commands.Cog):
    ...
    async def drink(self, ctx, *name):
        ...
        if not self.happy_hour:
            return await ctx.send(
                "Sorry, friend. Come back at 5pm when we're open.  Feel free to take some complimentary `!coffee`."
            )
    ...
    async def cocktail(self, ctx, *name):
        ...
        # get drink by name
        r = requests.get(
            "https://www.thecocktaildb.com/api/json/v1/1/search.php?s=%s&a=%s"
            % (drink_name, alcoholic)).json()
    ...
    async def i_drink(self, ctx, ingredient):
        ...
        drink_info = await self.get_drink_info(r['drinks'])
        await ctx.send(
            "You got it, boss. Let me see what I can make with that.")
        async with ctx.typing():
            await ctx.send(drink_info[0])
            await asyncio.sleep(random.randrange(4, 6))
            return await ctx.send(drink_info[1])
    ...
    @commands.command(
        aliases=['random_drink', 'rdrink', 'rand_drink', 'randdrink'])
    ...
    async def bardown(self, ctx):
        ...
        count_hours, count_minutes = countdown.seconds // 3600, (
            countdown.seconds // 60) % 60
        count_seconds = countdown.seconds - count_hours * 3600 - count_minutes * 60
        # build and send string
        ret_string = str(count_hours) + " hours, " + str(
            count_minutes
        ) + " minutes and " + str(
            count_seconds
        ) + " seconds until Obotma's Shabby but Classy Cabaret opens. Have come complimentary `!coffee` in the meantime.\nFree drinks for our first week's grand opening!"
    ...
    async def coffee(self, ctx):
        ...
        if r and 'file' in r:
            ...
            return await ctx.send(
                "Looks like we're all out of coffee. Sorry, boss.")
    ...
    async def get_drink_info(self, drinks):
        if drinks:
            ...
            info_string = "You got it, boss. Let me whip that up for you.\n" + "**" + drink[
                'strDrink'] + "** *(" + drink['strCategory'] + ")*, " + drink[
                    'strAlcoholic'] + "\n**Glass**: *" + drink[
                        'strGlass'] + "*\n" + "**Ingredients: **" + ", ".join(
                            ingredients) + "\n**Instructions**: " + drink[
                                'strInstructions'] + "\n Here you are. Enjoy!"
            ...
            return (
                "None of that behind the bar, boss. Can I get you something else?",
                "https://tenor.com/view/i-must-be-of-service-chris-conner-poe-altered-carbon-i-must-provide-service-gif-17622348"
            )
    ...
    async def bar_open(self):
        ...
        self.happy_hour = True
        await channel.send(
            "Bar is open, gents. Come order your drinks and stay a while.")
    ...
    async def bar_close(self):
        ...
        # channel = gl.bot.get_channel(887682725375528963) # testing chat
        await channel.send(
            "Bar's closed, everyone. You don't have to go home, but you can't stay here. Except for you, Drew. You can stay as long as you want."
        )
        self.happy_hour = False
    ...
    async def before_bar_close(self):
        ...
        bar_closes = datetime(cur_time.year,
                              cur_time.month,
                              cur_time.day + 1,
                              4,
                              0,
    ...
    async def add_stat(self, id):
        id = str(id)
        if id not in gl.db["drink_stats"]:
            gl.db['drink_stats'][id] = 1
        else:
            gl.db['drink_stats'][id] += 1
        await gl.update_db()
